...
def recognize(path_to_read,user_name,start,case):
	...
	thresh= ~thresh1  #convert black to white and white to black
	'''while True:
	...
	for (i, c) in enumerate(cnts):
		...
		if cropped.shape[0]>=4 and cropped.shape[0]<=400 and cropped.shape[1]>=2 and cropped.shape[1]<=400 and cropped.shape[1]*cropped.shape[0]>1000:
			...
			path_to_write=os.path.join("E:\\txttohandwritting-master\\train",user_name)
			if not os.path.exists(path_to_write):
				os.makedirs(path_to_write)

			#cv2.imwrite(path_to_write,output)
			cv2.imwrite(path_to_write+"\\%s"%str(start)+case+".png",output)
			...
			cv2.imshow("rec",thresh1)
			cv2.waitKey(100)
			digit_car=ord(start[0])
			digit_car=digit_car+1
			start=chr(digit_car)
	# show the output image 
	#cv2.imshow("Image", thresh1)

list_of_path=[]
list_of_path.append("E:\python-class-project\\txttohandwritting-master\images\\a_n.jpg")
list_of_path.append("E:\python-class-project\\txttohandwritting-master\images\\o_z.jpg")
list_of_path.append("E:\python-class-project\\txttohandwritting-master\images\\A_M.jpg")
list_of_path.append("E:\python-class-project\\txttohandwritting-master\images\\N_Z.jpg")
list_of_path.append("E:\python-class-project\\txttohandwritting-master\images\\0_9.jpg")

def genrate_char(list_of_path):
	new_path=(list_of_path[0])
	username='rahul'
	recognize(new_path,username,'a','')

	new_path=(list_of_path[1])
	recognize(new_path,username,'o','')

	new_path=(list_of_path[2])
	recognize(new_path,username,'a','upper')

	new_path=(list_of_path[3])
	recognize(new_path,username,'n','upper')

	new_path=(list_of_path[4])
	recognize(new_path,username,'0','')

	return (os.path.join("E:\\txttohandwritting-master\\train",username))
if __name__ == "__main__":
	genrate_char(list_of_path)
